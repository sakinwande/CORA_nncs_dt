classdef neurNetContrSysDt < contDynamics
...
methods
    ...
    % class constructor
    function obj = neurNetContrSysDt(varargin)
...
%         if nargin == 1 && isa(varargin{1},'neurNetContrSysDt')
...
% check correctness of input arguments
    ...
    if CHECKS_ENABLED && n_in > 0
        ...
        % check data types
        inputArgsCheck({ ...
            {sys,'att',{'nonlinearSysDT','nonlinParamSys'}},...
...
% compute properties of neurNetContrSys object
    ...
    elseif isa(sys, 'nonlinearSysDT')
        f = @(x,u) [sys.mFile(x(1:n), [x(n+1:n+m); u]); zeros(m, 1)];
        name = [sys.name, 'Controlled'];
        sys = nonlinearSysDT(name, f, dt, n+m, max(1, sys.nrOfInputs-m));
